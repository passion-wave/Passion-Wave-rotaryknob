...
ESPHOME_PLATFORM_KEY="${ESPHOME_PLATFORM//\//-}"
PLATFORMIO_CACHE="${PLATFORMIO_CACHE:-${REPO_ROOT}/.esphome_cache/${ESPHOME_PLATFORM_KEY}/platformio}"
CCACHE_CACHE="${CCACHE_CACHE:-${REPO_ROOT}/.esphome_cache/${ESPHOME_PLATFORM_KEY}/ccache}"
ESPHOME_CACHE="${ESPHOME_CACHE:-${REPO_ROOT}/.esphome_cache/${ESPHOME_PLATFORM_KEY}/esphome}"
detect_serial_port() {
  local candidates=()
  shopt -s nullglob
